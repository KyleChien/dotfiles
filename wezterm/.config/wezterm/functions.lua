...
-- Truncates a path to max_len by keeping the first component, inserting "...",
-- then greedily appending trailing components that still fit.
-- e.g. /Users/kylechien/dotfiles/wezterm → /Users/.../wezterm
function F.truncate_path(path, max_len)
	...
	for i = #parts - 1, 2, -1 do
		...
		if string.len(head .. "/.../" .. candidate) <= max_len then
			tail = candidate
		else
			break
		end
